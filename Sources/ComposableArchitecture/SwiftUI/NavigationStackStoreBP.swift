...
public struct _NavigationLinkStoreContentBP<State, Label: View>: View {
  let state: State?
...
extension NavigationStackBackport.NavigationLink {
  ...
  where Label == _NavigationLinkStoreContentBP<P, L> {
    @Dependency(\.stackElementID) var stackElementID
    self.init(value: state.map { Component(id: stackElementID(), element: $0) }) {
        _NavigationLinkStoreContentBP<P, L>(
  ...
  where Label == _NavigationLinkStoreContentBP<P, Text> {
